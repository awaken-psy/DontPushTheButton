...
{
    /// <summary>
    /// 推动能力（M3.1 磁力枪·抓取搬运型，持续型）。
    /// 切换式：按推动键 → Idle 抓取前方 grabRange 内 Pushable（普通1个 / 超载范围内上限 maxCarryCount）→ Carrying；
    /// 持有期间物体每帧吸附玩家前方 carryDistance（多个横向分布）；再按 → 释放（原地放下）→ Idle。
    /// 非物理（直接 setPostion，与 M2.1 H2 一致）。超载搬多个 = 抓取成功时 ChargeOverload 扣一次。
    /// </summary>
    public class PushAbility : AbilityBase
    {
        ...
        private void TryGrab(IAbilityContext ctx)
        {
            ...
            // 收集范围内 Pushable
            _hitBuffer.Clear();
            foreach (var hit in Physics.OverlapSphere(center, _pushTuning.GrabRange))
            ...
            int count = overload
                ? Mathf.Min(_pushTuning.MaxCarryCount, _hitBuffer.Count)
                : 1;
        ...
        private void UpdateCarryPositions(IAbilityContext ctx)
        {
            ...
            Vector3 basePos = ctx.Body.position + fwd * _pushTuning.CarryDistance;
