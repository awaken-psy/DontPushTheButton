---
description: 本项目 Claude 的 git 执行准则（分支/提交动作/提交信息风格/Unity 纪律/PR；提交时机由 milestone-rules 管）
---
# Git 执行准则（Claude）
...
## 2. 提交动作（时机见 milestone-rules）
- **何时提交由 `.claude/rules/milestone-rules.md` 决定**（当前约定：每完成一个子任务 `M{N}.x` 提交一次）；本规则只管提交的**动作规范**，不管时机。
- 例外：用户当次明确说「先别提交 / 别 push / 攒一起」，遵从用户，不提交。
- 每次提交前 `git status` 核对（见第 5 节），确认无 `Library/` 等误入、`.meta` 与资产成对。
- 文档与代码改动**分开 commit**（见第 4 节）。
